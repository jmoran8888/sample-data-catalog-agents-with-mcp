...
For local testing and development, follow the "Local Development Setup" section below.

### 2. AWS Deployment with Terraform (Original)

Deploy using Terraform with AgentCore toolkit:
- Single command deployment: `python setup/deploy_aws.py`
- AgentCore toolkit handles MCP server deployment
- See "AWS Deployment" section below for complete instructions

### 3. AWS Deployment with CDK v2 (New - Semantic Layer Pattern)

Deploy using AWS CDK with semantic-layer MCP pattern:
- **Location**: `infrastructure-v2/` directory
- **Pattern**: AgentCore Runtimes (no toolkit)
- **Multi-Region**: Supports us-east-1, us-west-2, eu-west-1
- **Deploy**: `python setup/deploy_cdk.py` (automated wrapper script)
- **Cleanup**: `python setup/cleanup_cdk.py` (destroys all CDK resources)
- **Manual Deploy**: `cd infrastructure-v2 && npx cdk deploy --all`

**Documentation:**
- [infrastructure-v2/README.md](infrastructure-v2/README.md) - Complete CDK guide
- [infrastructure-v2/DEPLOYMENT.md](infrastructure-v2/DEPLOYMENT.md) - Deployment steps

**Key Differences from Terraform:**
- Uses CDK instead of Terraform
- Implements semantic-layer pattern for MCP servers
- Direct AgentCore Runtime API (no toolkit)
- Multi-region support with automatic AZ detection
- Native CDK Docker image building

**Security Note:** Review [SECURITY.md](SECURITY.md) before deploying.
