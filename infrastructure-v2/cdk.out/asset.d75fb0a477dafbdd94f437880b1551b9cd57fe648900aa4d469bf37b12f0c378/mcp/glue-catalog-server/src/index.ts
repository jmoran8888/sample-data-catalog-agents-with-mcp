...
// Define the path to the Python script that will invoke the Glue catalog agent
const pythonScript = path.join(projectRoot, 'mcp', 'glue-catalog-server', 'invoke_glue_agent.py');
...
// Function to execute the Python script with the Glue catalog agent
async function invokeGlueAgent(toolName: string, params: Record<string, any> = {}): Promise<any> {
...
class GlueCatalogServer {
  ...
  private setupToolHandlers() {
      ...
      tools: [
        {
          ...
          description: 'List all databases in the AWS Glue catalog',
          inputSchema: {
            type: 'object',
            properties: {},
            required: [],
        ...
        },
        {
          ...
          description: 'List all tables in a specific AWS Glue database',
          inputSchema: {
            type: 'object',
            properties: {
              database_name: {
        ...
        {
          name: 'get_glue_table_details',
          description: 'Get detailed information about a specific table in the AWS Glue catalog',
          inputSchema: {
            type: 'object',
            properties: {
              database_name: {
        ...
        {
          name: 'search_glue_tables_by_name',
          description: 'Search for tables by name pattern in the AWS Glue catalog',
          inputSchema: {
            type: 'object',
            properties: {
              name_pattern: {
        ...
        {
          name: 'search_glue_tables_by_column',
          description: 'Search for tables containing columns matching the pattern in the AWS Glue catalog',
          inputSchema: {
            type: 'object',
            properties: {
              column_pattern: {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        const result = await invokeGlueAgent(toolName, args);
        ...
        console.error(`Error calling Glue catalog agent:`, error);
        return {
          content: [
            {
              type: 'text',
              text: `Failed to call Glue catalog agent: ${error instanceof Error ? error.message : String(error)}`,
            },
          ],
          isError: true,
        };
  ...
  async run() {
    ...
    console.error('Glue Catalog MCP server running on stdio');
  }
}

const server = new GlueCatalogServer();
