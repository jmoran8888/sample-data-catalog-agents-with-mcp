...
def main():
    ...
    if run_command('python setup/setup_unity_sample_data.py', cwd=project_root, check=False):
        print(f"{GREEN}✓ Unity Catalog sample data initialized{RESET}")
    else:
        print(f"{YELLOW}⚠ Unity Catalog sample data initialization had issues (check logs){RESET}")
